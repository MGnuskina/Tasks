...
namespace PPersistence
{
    public class PersonRepositoryAdo : IRepository<Person>
    {
            ...
            try
            {
                ...
                while (reader.Read())///Mapper seperatly
                {
                    result.Add(new Person()
                    {
                        ...
                        phonenumbers = new List<PhoneNumbers>() { new PhoneNumbers() { ID = reader.GetInt32(4), PersonID = reader.GetInt32(0), PhoneNumber = reader.GetString(5), PhoneNumberType = reader.GetString(6) } }
                    });
                }
            }
            catch { }
        ...
        public Person GetByID(int id)
        {
            string queryString = "SELECT Person.ID, Person.FirstName, Person.LastName, Person.Age," +
                "PhoneNumbers.ID, PhoneNumbers.PhoneNumber, PhoneNumbers.PhoneNumberType " +
                "FROM [Person] INNER JOIN [PhoneNumbers] ON Person.ID=PhoneNumbers.PersonID WHERE Person.ID LIKE @ID";
            ...
            try
            {
                ...
                result.phonenumbers = new List<PhoneNumbers>() { new PhoneNumbers() { ID = reader.GetInt32(4), PersonID = reader.GetInt32(0), PhoneNumber = reader.GetString(5), PhoneNumberType = reader.GetString(6) } };
            }
            catch { }
            return result;
        }
